...
public class RefundController {
    ...
    public ResponseEntity<ApiResponse<RefundAdminReadResponseDto>> getRefundStatus(@PathVariable UUID paymentId,
                                                                                   @RequestHeader(value = "X-User-UUID") UUID userId,
                                                                                   @RequestHeader(value = "X-User-Role") String role) {
        RefundAdminReadResult result = refundService.getRefund(paymentId, userId, role);
        RefundAdminReadResponseDto responseDto = RefundReadResponseMapper.toResponse(result);
    ...
    //환불 검색
    @GetMapping("/search")
    public ResponseEntity<ApiResponse<Page<RefundAdminReadResponseDto>>> searchRefund(@RequestBody @Valid RefundSearchRequestDto requestDto,
                                                                                      @RequestHeader(value = "X-User-UUID") UUID userId,
                                                                                      @RequestHeader(value = "X-User-Role") String role) {
        RefundSearchCommand command = RefundSearchMapper.toCommand(requestDto);
        Page<RefundAdminReadResult> result = refundService.searchRefund(command, userId, role);
        Page<RefundAdminReadResponseDto> responseDto = RefundReadResponseMapper.toPageResponse(result);
        return ResponseEntity.status(200).body(ApiResponse.success(responseDto));
    }
